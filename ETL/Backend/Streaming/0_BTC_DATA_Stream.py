...
def merge_into_single_csv(date_list,init=False,debug=False,typeOfData=['klines']):    
    ...
    for directory in typeOfData:
        try:
            ...
            #base_df['open_time'] = base_df['open_time'].apply(lambda x: x/1 if isinstance(x,int) else x)
          

            #base_df['open_time']=pd.to_datetime(base_df['open_time'],unit='ns')
            #print('NYAH!')
            #base_df['open_time']= base_df['open_time'].apply(lambda x: pd.to_datetime(x['open_time'] , unit='ms') if isinstance(x,int) else x)
            #base_df['open_time']=[pd.to_datetime(base_df['open_time'].iloc[i],unit='ms') for i in range(base_df.shape[0]) else]
            import datetime
            #base_df['open_time']= base_df['open_time'].apply(lambda x: int(x[:-2])  if len(x) <= 15 else x)
            #base_df['open_time']= base_df['open_time'].apply(lambda x: int(x[:-2])  if isinstance(x,int) else x)
            base_df['open_time']= base_df['open_time'].apply(lambda x: pd.to_datetime(x, unit='ms') if isinstance(x,int) else x)
            #pd.to_datetime(base_df['open_time'].iloc[1],unit='ms')
            ...
            base_df.reset_index(drop=True,inplace=True)
            base_df.to_csv(f'./../../Database/Futures_um/{directory}/Full_Data_2klines.csv',index=False)
            continue
